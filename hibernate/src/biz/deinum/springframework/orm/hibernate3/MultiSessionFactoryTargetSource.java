 ...
 * context.Very useful if we have one application which points to different
 * databases, for different context (i.e. clients).
 * 
 * @author Marten Deinum
 * @see TargetSource
 */

public class MultiSessionFactoryTargetSource implements TargetSource, InitializingBean {

	private Logger logger = LoggerFactory
			.getLogger(MultiSessionFactoryTargetSource.class);

	private SessionFactory defaultSessionFactory;

	private final Map sessionFactories = Collections
	 ...
	 * 
	 * First we lookup the context name from the <code>ContextPathHolder</code>
	 * this context name is used to lookup the desired sessionfactory. When none
	 * is found we return the default sessionfactory, which must be
	 * configured!
	 * 
	...
	public Object getTarget() throws Exception {
		...
		if (sessionObject == null) {
			sessionObject = defaultSessionFactory;
	...
	public void setDefaultSessionFactory(final SessionFactory defaultSessionFactory) {
		this.defaultSessionFactory = defaultSessionFactory;
	}

	public void afterPropertiesSet() throws Exception {
		Assert.notNull(defaultSessionFactory, "The defaultSessionFactory must be set!");
	}
}